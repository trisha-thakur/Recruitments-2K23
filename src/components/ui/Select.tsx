...
const OptionBox: React.FC<{ label: string; onClick: () => void; isSelected: boolean }> = ({label, onClick, isSelected,}) => {
  return (
    <button className={`p-4 max-w-sm border rounded-lg hover:border-secondary ${isSelected ? 'bg-secondary' : ''}`} onClick={onClick}>{label}</button>
  );
};
...
const Select: React.FC<{
  ...
  selectedOption: string | null} > = ({options, style, setSelectedOption, selectedOption}) => {
